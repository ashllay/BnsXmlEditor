...
namespace Core
{
    ...
    public class BnsDat
    {
        ...
        public byte[] XOR_KEY = new byte[16] { 164, 159, 216, 179, 246, 142, 57, 194, 45, 224, 97, 117, 92, 75, 26, 7 }; // A4 9F D8 B3 F6 8E 39 C2 2D E0 61 75 5C 4B 1A 07
        ...
        public byte[] Deflate(byte[] buffer, int sizeCompressed, int sizeDecompressed)
        {
            ...
            //if (tmp.Length != sizeDecompressed)
            {
                ...
                //MessageBox.Show("run out of sizeDecompressed: " + tmp.Length + " - " + sizeDecompressed);
        ...
        public void Extract(string FileName, bool is64 = false)
        {
            ...
            //Console.WriteLine("Signature: {0}, Version: {1}", Encoding.ASCII.GetString(Signature), Version);
            //MessageBox.Show(string.Format("Signature: {0}, Version: {1}", Encoding.ASCII.GetString(Signature), Version));
        ...
        public void Compress(string Folder, bool is64 = false, int compression = 9)
        {
            ...
            //byte[] Signature = Encoding.ASCII.GetBytes("UOSEDALB");
            bw.Write(Signature);
            int Version = 2;
            bw.Write(Version);
    ...
    class BXML
    {
        ...
        public BXML_TYPE DetectType(Stream iStream)
        {
            ...
            iStream.Read(Signature, 0, 8);
            iStream.Position = offset;

            BXML_TYPE result = BXML_TYPE.BXML_UNKNOWN;

            if (BitConverter.ToString(Signature).Replace("-", "").Replace("00", "").Contains(BitConverter.ToString(new byte[] { (byte)'<', (byte)'?', (byte)'x', (byte)'m', (byte)'l' }).Replace("-", "")))
            {
