...
namespace BnsXmlEditor
{
    public partial class EditForm : Form
    {
        ...
        private void SaveTranslate(bool isGg)
        {
            if (elements.SelectedIndices.Count > 0)
            {
                _xmlFile.ElementUpdated(_listViewItems[elements.SelectedIndices[0]], translatedText.Text, isGg);
        ...
        private void btnNotTran_Click(object sender, EventArgs e)
        {
            ClearSelectedItem();
            searchQuery.Text = string.Empty;
            UpdateItems(_xmlFile.GetUnTranElements(comboBox1.SelectedItem.ToString()).ToList());
            //MessageBox.Show(comboBox1.SelectedItem.ToString());
        }

        private void btnSearchAllGtran_Click(object sender, EventArgs e)
        {
            ClearSelectedItem();
            searchQuery.Text = string.Empty;
            UpdateItems(_xmlFile.GetGTranElements().ToList());
        }

        private void gTran_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            translatedText.Text = Translate(translatedText.Text, "cn-zh", "en");
            SaveTranslate(true);
        }

        private void resetTran_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            if (elements.SelectedIndices.Count > 0)
            {
                translatedText.Text = _xmlFile.ElementReset(_listViewItems[elements.SelectedIndices[0]]);
            }
        }
        public static string Translate(string text, string from, string to)
        {
            ...
            try
            {
                ...
                //https://translate.google.com/#zh-CN/en/%E5%A5%BD
                string url = string.Format(@"https://translate.google.com/m?hl=en&sl={0}&tl={1}&ie=UTF-8&prev=_m&q={2}", from, to, Uri.EscapeUriString(text));
